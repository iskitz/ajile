...
| Modified: Tuesday,   November  13, 2012    [2012.11.13 - 08:30:08 PST]
|+-----------------------------------------------------------------------------+
|
...
(function/*com.iskitz.ajile*/(VERSION, undefined)
{
   ...
   function AddImportListener(moduleName, listener)
   {
      ...
      {
         window.setTimeout (function(){ $listener (moduleName); }, 0);
         return true;
      }
      ...
      if (!moduleName)
         if (processed.getSize() > 0 || pendingImports.getSize() == 0)
            for (var importee in processed.getAll())
               window.setTimeout (function(){ $listener (importee); }, 0);
