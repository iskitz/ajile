...
(function /*Notify users of ajile's default configuration*/ (global, undefined) {
    ...
    var domain = global.document ? document.domain : undefined;

    var logConfig =
        { 'ajile.net'           : true  // Enable  logging ajile's configuration for ajile.net
        , 'cdn.jsdelivr.net'    : true  // Enable  logging ajile's configuration for jsDelivr
        , 'cdnjs.cloudfare.com' : true  // Enable  logging ajile's configuration for cdnjs
        , 'add.your.domain.here': false // Disable logging ajile's configuration for your site.
        }
        ;

    if (!domain || !logConfig [domain]) return;
